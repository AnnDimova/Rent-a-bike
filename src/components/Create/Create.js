...
export const Create = () => {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    const bikeDataForm = Object.fromEntries(new FormData(e.target));
    const bikeData = { ...bikeDataForm, ownerEmail: auth.email };
    await bikeService.create(bikeData, auth.accessToken);
    await sleep(250);
    navigate("/catalog");
  };
